...
export default function IncidentTable({ data, title = "Assigned Complaints", showViewAll = true, itemsPerPage = 10 }: IncidentTableProps) {
          ...
          <tbody className="divide-y divide-slate-100">
            {paginatedData.length > 0 ? paginatedData.map((item) => (
              <tr key={item.id} className="hover:bg-blue-50/30 transition-colors">
                ...
                <td className="px-6 py-4 text-slate-600">{item.issueCategory}</td>
                <td className="px-6 py-4 text-slate-900 font-medium">{item.vehicleId || 'N/A'}</td>
