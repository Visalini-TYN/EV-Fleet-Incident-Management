...
export default function ManagerVehiclePage() {
  ...
  const handleAddVehicle = async () => {
    if (!make || !model || !licensePlate || !vin) {
    ...
    try {
      await vehiclesApi.create({
        ...
        batteryCapacityKwh: parseFloat(battery),
        chassisNo,
      });
        ...
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 text-slate-500 text-lg">
              Manage and monitor your electric vehicle fleet assets.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredVehicles.map(
                (vehicle: any, index: number) => (
                  <div
                    key={index}
                    onClick={() => navigate(`/manager/vehicle/${vehicle.id}`)}
